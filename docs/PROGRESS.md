...
**Current milestone:** M13 — Docker/operator docs — COMPLETE (offline artifact validation; Docker Compose syntax validated). M0–M13 are complete; no later milestone is scheduled.

**State:** M13 delivered the ROADMAP deployment/operator items behind ADR-040: a single non-root `python:3.12-slim` runtime image, Compose service with local host-port publication, named `gateway-data` volume, `/health` healthcheck, runtime-only `.env` injection, container-safe `.env.example` guidance, and `docs/OPERATIONS.md` covering build/start, health, Qwen Code setup, logs, restart/recreate, diagnostics, volume boundaries, and troubleshooting. The image never contains `.env`, tokens, cookie files, tests, or local virtualenv artifacts. The container binds `0.0.0.0` internally while Compose defaults to `127.0.0.1:8000` on the host. Runtime credentials remain outside the image. The named volume currently protects opt-in diagnostics only; conversations, account state, and metrics remain in-memory by design.

**Previous milestone:** M12 — Admin UI — COMPLETE (offline; all admin surfaces deterministic and pinned by tests).

**M12 state (superseded by M13 deployment docs only):** M12 delivered the ROADMAP admin-UI items behind ADR-039: one self-contained HTML page at `GET /admin` (inline CSS + vanilla JS, zero external assets, no build step — works with the gateway fully offline) that is a pure stateless CLIENT of the admin JSON endpoints; additive read views `GET /admin/summary` (the SAME health-payload closure `/health` uses + fleet account counts by state + conversations + active sessions + uptime + six key metrics), `GET /admin/sessions` (per-conversation binding METADATA — linked flag, status, message/tool-call counts; never message content or tool arguments), `GET /admin/settings` (effective config echo — secrets as presence only: `configured`/`open`/`unset`, never values); account lifecycle mutations `POST /admin/accounts/{id}/disable|enable|reset` as thin projections over the M10 router seams (`disable` = `set_enabled(False)` — flips the flag AND releases conversation links; `enable` = `set_enabled(True)` — flag ONLY, invalid state survives until reset; `reset` = enabled + healthy + cleared cooldown + cleared consecutive_failures); unknown account id → 404 `ACCOUNT_NOT_FOUND` in the OpenAI envelope. Credentials stay STRUCTURALLY masked — no admin view ever touches a `SecretStr` value. Core API independence proven by construction: every M12 route is purely additive, `/v1/*` + `/health` + `/admin/metrics` + `/admin/accounts` are byte-identical, and the suite grew 517 → 535 with ZERO re-pins. Auth stance unchanged: all `/admin/*` remain unauthenticated (local bind 127.0.0.1 is the boundary — recorded residual risk). Runtime account ADD/REMOVE stays deferred (would move credentials across the admin boundary); settings remain env-derived (restart applies).

**Previous milestone:** M11 — Session failover — COMPLETE (offline; failover behavior is deterministic and pinned by tests).

**M11 state (superseded by M12 only where noted above):** M11 delivered the ROADMAP session-failover items behind ADR-038: bounded IN-REQUEST session failover for FINAL consequence-bearing pre-byte failures (401-class `AUTH_INVALID` and 429-class `RATE_LIMITED` — exactly the account-scoped categories; network/5xx/protocol/cloudflare failures are fleet-wide on the shared upstream and never fail over). On such a failure the gateway records the M10 account consequence, establishes EXACTLY ONE failover session on another usable account (`select_for_new_conversation` after the consequence — the failed account can never be re-picked), REHYDRATES the request's FULL canonical history through the identical rebuild compilation path (tool history/IDs preserved by construction — envelope blocks per ADR-034, `[tool result]` blocks with names resolved through the tool-call ID index), and re-runs the same turn there with `parent_message_id=None`. Failover sites: session creation, buffered tool turns (both response modes), non-stream drains, streaming priming — everything pre-byte; mid-stream NEVER fails over (committed-stream rule). Error policy: failover is best-effort transparency — when it cannot establish (no usable account, or the target's session creation fails), the ORIGINAL failure surfaces byte-identical; a failed re-run surfaces ITSELF with the failover account's consequence recorded. A committed failover turn binds the conversation to the failover account (never migrated back). New `session_failovers` metrics marker (counted when a failover session is ESTABLISHED, before the re-run) + one secret-free INFO log line. Additive `FakeBackend(create_failures=...)`. New suite `tests/test_m11_failover.py` (13 tests). Suite 504 → 517 passed with exactly TWO planned re-pins of M10 flow tests that flip BY DESIGN (the failing request now succeeds transparently via failover; the 429 variant re-pinned with a disabled second account to keep the fleet-exhaustion + bound-rebuild pins). Single-account deployments stay byte-for-byte unchanged.

**Previous milestone:** M10 — Multi-account router — COMPLETE (offline; routing/failover behavior is deterministic and pinned by tests).
...
- M10 (2026-08-15): multi-account router — in-memory account registry (`app/accounts.py`: `AccountRecord` — id/label/backend/enabled/health_status/cooldown_until/consecutive_failures/last_used_at/timestamps, NO credential on the record; states healthy/cooldown/invalid, `disabled` derived from `enabled=False`), thread-safe `AccountRouter` (least-active selection DERIVED from the conversation store per call — self-healing under LRU eviction; unused accounts first; ties → least recently used → config order; LAZY cooldown promotion with injectable clock, no background thread; zero usable accounts → 429 `RATE_LIMITED` when any account is still cooling, else 502 `AUTH_INVALID`, secret-free message), sticky conversation account (a live backend link keeps its account — cooldown INCLUDED; invalid/disabled releases the link; a dead-link rebuild prefers its bound account while that account stays sticky-usable, so a final 429 never blocks the ADR-020 rebuild path), account consequences on FINAL failures only (post-M9-budget: `AUTH_INVALID` → invalid + release ALL bound conversation links; `RATE_LIMITED` → cooldown `now + DSQG_ACCOUNT_COOLDOWN_SECONDS` blocking NEW conversations only; other categories counter-only; success → healthy, cooldown/counters cleared), `DSQG_ACCOUNT_TOKENS` config (accounts `acct-1..N` in config order; mutually exclusive with `DEEPSEEK_AUTH_TOKEN`; empty/duplicate entries and cookies+multi are fail-closed `ConfigError`s naming variables, never values; `DSQG_ACCOUNT_COOLDOWN_SECONDS` default 300 > 0; single-account config byte-for-byte unchanged — the 458-test regression pin held with ZERO re-pins), per-account backend isolation (each account owns its `DeepSeekWebBackend` → own vendored client, PoW solver, call gate; cross-account parallelism safe per ADR-037 point 2), `create_app(..., router=)` wiring (`app.state.router`; an injected backend wraps into the one-account `default` router), masked read-only `GET /admin/accounts` (unauthenticated like `/admin/metrics`; structurally secret-free), fleet-aware `/health` (`ok` = backend ready AND ≥1 enabled account not invalid), "encrypted credentials" interpreted as in-memory `SecretStr` masking (at-rest encryption deferred to the persistence milestone — SECURITY.md), operator `set_enabled`/`reset` methods unit-tested (HTTP surface is M12), M11 boundary honored (no mid-conversation migration, no failover marker). ADR-037. 46 new tests in `tests/test_m10_accounts.py`. Suite 458 → 504 passed.
- M11 (2026-08-15): session failover — bounded in-request failover (ADR-038) for FINAL consequence-bearing PRE-byte failures (`FAILOVER_CATEGORIES` = `AUTH_INVALID` / `RATE_LIMITED` only — all accounts share ONE upstream, so fleet-wide categories would only double the load): the failed account takes its M10 consequence, then ONE `select_for_new_conversation` + `create_session` (inside `with_transport_retry`) on another usable account — selection-after-consequence structurally excludes the failed account; the request's FULL canonical history is REHYDRATED through `_compile_turn` — the SAME compilation shared with `_prepare_turn`, so rebuild and failover prompts cannot drift, and tool history/IDs survive by construction (ADR-034 envelope blocks + `[tool result]` blocks + the tool-call ID index); the same turn re-runs on the fresh session with `parent_message_id=None`. Failover sites: `_prepare_turn` session creation (both resolution branches), buffered tool turns (stream + non-stream via `_run_tool_turn_with_failover`), non-stream plain drain, streaming priming (first event) — mid-stream NEVER (committed-stream rule). Error policy: establishment failure → ORIGINAL error surfaces byte-identical (single-account deployments unchanged); re-run failure → surfaces ITSELF, consequence recorded on the failover account; failover never chains. Rebind: a committed failover turn binds the conversation to the failover account (`Conversation.status` stays `active`, no new status value, no migration back). Metrics marker `session_failovers` (`MetricsCollector.record_session_failover`, counted at ESTABLISHMENT before the re-run) + one INFO log line `session failover: account <id> -> <id> (category=<cat>)` — ids and category only, never secrets. Additive `FakeBackend(create_failures=[...])` queue consumed by `create_session`. No new config variables; `/health` and `/admin/accounts` unchanged. 13 new tests in `tests/test_m11_failover.py`; exactly TWO M10 flow tests re-pinned in place (by design: `test_final_401_invalidates_and_rebuilds_elsewhere` now asserts the transparent in-request success + marker + rebind; `test_final_429_cools_account_and_rebuild_stays_bound` re-pinned with a DISABLED second account so the fleet-exhaustion error policy and the ADR-037 bound-rebuild refinement stay pinned). Suite 504 → 517 passed.
- M13 (2026-08-15): Docker/operator deployment (ADR-040) — single non-root `python:3.12-slim` runtime image (`Dockerfile`) installs the package from `pyproject.toml`, copies only `app/`, `vendor/`, and packaging metadata, and runs `python -m app.main` as user `gateway` (uid 10001). `compose.yaml` provides one `gateway` service with `restart: unless-stopped`, runtime-only `.env` injection, internal `GATEWAY_HOST=0.0.0.0`, host publication `${GATEWAY_PUBLISH_HOST:-127.0.0.1}:${GATEWAY_PUBLISH_PORT:-8000}:8000`, named `gateway-data` volume at `/var/lib/deepseek-qwen-gateway`, and a Python stdlib `/health` healthcheck (30s interval, 10s timeout, 3 retries, 20s start period). `.dockerignore` excludes `.env`, tokens, cookie files, virtualenvs, tests, and local artifacts; `.env.example` documents host publish knobs while retaining native-run loopback defaults. `docs/OPERATIONS.md` covers prerequisites, `docker compose up -d --build` / `docker compose up -d` exit path, fake-backend smoke, Qwen Code `baseUrl: http://127.0.0.1:8000/v1`, lifecycle/logs/recreate, diagnostics and volume boundaries, port/auth/upstream troubleshooting. No credential is copied into the image; named volume protects diagnostics only — conversations, account state, and metrics remain in-memory by design. Five static artifact tests in `tests/test_m13_deployment.py`; Compose config validation passed; full suite 535 → 540 passed, 3 deselected.
- M12 (2026-08-15): admin UI — one self-contained HTML page at `GET /admin` (`include_in_schema=False`; inline CSS + vanilla JS, zero external assets/build step, dark theme, five tabs — Dashboard / Accounts / Sessions / Metrics / Settings — with 5-second auto-refresh) that is a pure stateless CLIENT of the admin JSON endpoints; the page itself is tested to contain no external `src=`/`href=` URLs and to reference exactly the JSON endpoints it uses. Additive read views built as PURE functions in `app/admin.py` (unit-testable without HTTP): `build_summary` → `GET /admin/summary` (the /health payload — produced by a `_fleet_health_payload` closure in `create_app` shared VERBATIM with the `/health` handler so the dashboard card and the probe can never diverge — plus `backend_type`, fleet counts `by_state` healthy/cooldown/invalid/disabled, conversations, `active_sessions` (conversations with a live backend link), `uptime_seconds`, and a six-key metrics subset {requests, backend_attempts, backend_failures, transport_retries, session_failovers, tool_turns}); `build_sessions_view` → `GET /admin/sessions` (rows newest-updated-first: conversation_id, backend_account_id, backend_session_id, linked, status, message_count, tool_call_count (derived), created/updated_at — binding METADATA only, never message content or tool arguments); `build_settings_view` → `GET /admin/settings` (backend_type, model_id, host/port, `gateway_auth` presence-only configured/open/unset, accounts mode single/multi + count, diagnostics enabled+dir, reliability knobs — secrets as PRESENCE only). Lifecycle mutations: `POST /admin/accounts/{id}/disable|enable|reset` — thin projections over the M10 router seams: disable = `set_enabled(id, False)` (flag + release ALL conversation links), enable = `set_enabled(id, True)` (flag ONLY — an invalid account stays invalid until reset), reset = `router.reset(id)` (enabled + healthy + cleared cooldown + cleared consecutive_failures); each responds `{"account": <updated masked row, same shape as /admin/accounts entries>}`; unknown id → 404 `ACCOUNT_NOT_FOUND` (`invalid_request_error`) for all three actions. Credentials stay STRUCTURALLY masked — `AccountRecord` carries no credential field and no view builder ever touches a `SecretStr`; the masking is sweep-tested with distinctive prompt/tool-output/secret markers asserted absent from EVERY admin read endpoint. Core API independence proven by construction: all M12 routes are additive, `/v1/*` + `/health` + `/admin/metrics` + `/admin/accounts` byte-identical (the `/health` refactor extracted its payload computation into the shared closure with identical output — verified byte-equal in tests), and the suite grew 517 → 535 with ZERO re-pins. Auth stance unchanged (M9/M10 precedent): all `/admin/*` remain UNAUTHENTICATED — local bind 127.0.0.1 default is the boundary, recorded as residual risk in ADR-039. Runtime account ADD/REMOVE deferred (would move credentials across the admin boundary); settings stay env-derived (restart applies). 18 new tests in `tests/test_m12_admin.py`. ADR-039.
...
M10 suite (same day, ADR-037):
.venv\Scripts\python.exe -m pytest -q
458 -> 504 passed, 3 deselected (live tests excluded by default
marker). New: tests/test_m10_accounts.py (46 tests) — router unit
pins (registry validation: ≥1 account, unique ids, uniform
backend_type, positive cooldown; single()/default id; timestamp
stamping; selection: unused-first -> least-active -> LRU tie-break ->
config order, last_used stamping, LAZY cooldown expiry/promotion,
no-usable category split RATE_LIMITED-vs-AUTH_INVALID; consequences:
401-class invalid + bound-link release only, 429-class cooldown
window with links intact, other categories counter-only, success
clears cooldown/counters, unknown ids ignored; sticky eligibility:
healthy/cooldown OK — invalid/disabled/unknown rejected; set_enabled
link release + selection block, reset, unknown-id KeyError; masked
summary: derived states disabled > invalid > cooldown > healthy,
remaining seconds, store-derived active counts, exact key set),
config boundary (DSQG_ACCOUNT_TOKENS parse -> deepseek_accounts +
deepseek_web mirror, mutual exclusion, empty/trailing-comma/duplicate
entries, cookies incompatibility, cooldown parse matrix, fake backend
ignores tokens, secret masking in repr/JSON), build_router wiring
(fake/deepseek single -> default account, multi -> acct-1..N with
DISTINCT DeepSeekWebBackend instances, ConfigError paths), public API
(sticky under balancing pressure, cooldown blocks new conversations
but not sticky sessions, final-429 flow — cooldown + new-conversation
detour + bound rebuild + success clears the window, final-401 flow —
invalidation + rebuild on another account with the full-history
prompt + health stays ok, all-unusable deterministic 429/502,
/admin/accounts masked + unauthenticated, health not_ready when every
account is invalid, create_app backend/router exclusivity + the
single-account compat router). ZERO re-pins needed: the 458-test
regression pin passed intact after one ADR-037 point-2 refinement
(dead-link rebuild affinity, caught by the M9 rebuild-contract test).

M11 suite (same day, ADR-038):
.venv\Scripts\python.exe -m pytest -q
504 -> 517 passed, 3 deselected (live tests excluded by default
marker). New: tests/test_m11_failover.py (13 tests) — transparent
non-stream failover (401-class: failing request 200 on acct-2, marker
1, source invalid, target healthy, rebind + fresh session + parent
reset + full-history prompt), rehydration fidelity (tool history
re-rendered as ADR-034 envelope blocks, arguments byte-preserved in
canonical compact JSON, [tool result] blocks keep the ORIGINAL id and
resolve the tool NAME through the tool-call index — never "unknown"),
429-class failover AFTER the full M9 retry budget (source cools),
session-creation failover (new conversation: create fails on acct-1 ->
established on acct-2 before any turn), establishment failure ->
ORIGINAL error byte-identical + target-account consequence + marker 0,
non-consequence categories NEVER fail over (INTERNAL surfaces 500,
target untouched, source stays healthy), single-account deployments
byte-for-byte unchanged, failover NEVER chains (three accounts: re-run
failure surfaces on acct-2, healthy acct-3 untouched, marker 1), a
failed re-run surfaces ITSELF with the target's consequence (original
401 source invalid, re-run 429 -> 429 "second", target cooling,
marker 1), streaming priming fails over (SSE 200 + [DONE] from the
failover account), MID-stream failures NEVER fail over (200 + error
envelope + no [DONE], marker 0, source still takes its consequence),
buffered tool turns fail over in BOTH response modes (envelope answer
on the failover account -> standard tool_calls chunks/fields, rebind).
TWO planned M10 re-pins, in place: test_final_401_invalidates_and_
rebuilds_elsewhere (the failing request now succeeds transparently —
200 "R1" via failover, marker 1, rebind to acct-2; follow-up new-
conversation + /health assertions kept) and test_final_429_cools_
account_and_rebuild_stays_bound (re-pinned with acct-2 DISABLED so no
failover target exists: the original 429 surfaces unchanged, marker 0,
new conversations hit the fleet no-usable-account 429, and the ADR-037
bound-rebuild-on-cooldown refinement stays pinned on the retry).

M12 suite (same day, ADR-039):
.venv\Scripts\python.exe -m pytest -q
517 -> 535 passed, 3 deselected (live tests excluded by default
marker). New: tests/test_m12_admin.py (18 tests) — page is
self-contained (200 text/html, five tab names present, NO external
src=/href= URLs) and pure-client (references exactly the six admin
JSON endpoints it uses); /admin/summary shape + live counts after one
chat (health == /health payload, by_state healthy 2, conversations 1,
active_sessions 1, backend_attempts >= 2) and fleet-state tracking
(disabling the only account flips health ok False and by_state
disabled 1 inside the SAME summary); /admin/sessions empty-then-
metadata-only rows (conv_ id, account id, fake backend session id,
linked, status, message_count 5, derived tool_call_count 1) with a
masking SWEEP — distinctive prompt and tool-output markers asserted
absent from ALL SIX admin read endpoints; settings builder masking on
real SecretStr values (gateway key + two account tokens: json.dumps of
the view never contains "super-secret"), auth modes unset/open,
endpoint echo of every reliability knob with the gateway API key value
absent from response.text; lifecycle — disable releases conversation
links and the continuation rebuilds on acct-2 (fresh session +
full-history prompt), enable after disable restores routing (disabled
single account -> 502 AUTH_INVALID fleet error, zero sessions; enable
-> 200), enable does NOT clear invalid (scripted AUTH_INVALID -> 502 +
invalid survives enable; only reset restores healthy + clears
consecutive_failures + serves again), reset clears cooldown (three
scripted RATE_LIMITED turns absorbed by M11 failover -> acct-1 cooling;
reset -> healthy + cooldown_remaining 0.0), unknown account id -> 404
ACCOUNT_NOT_FOUND for all three actions, single-account "default"
lifecycle; core independence — full M12 surface exercised between two
chats (incl. disable+reset on acct-2) leaves the continuation 200,
/health byte-equal to before, /admin/accounts row key-set unchanged,
and /admin/metrics M9 keys present. ZERO re-pins: every pre-M12 pin
(517 tests) passed intact — the only /health touch was extracting its
payload computation into the closure shared verbatim with
/admin/summary (byte-equality pinned in the M12 suite itself).

M13 deployment artifact suite (same day, ADR-040):
.venv\Scripts\python.exe -m pytest -q tests/test_m13_deployment.py
5 passed in 0.11s

The five offline tests pin the Dockerfile base image, package install,
non-root uid/user and entrypoint; Compose runtime-only `.env` injection,
0.0.0.0 internal bind, local host publication, named volume, and Python
stdlib `/health` healthcheck; `.dockerignore` secret/local-artifact
exclusions; operator docs exit path and Qwen Code `/v1` setup; and
container-related `.env.example` knobs. `docker compose config --quiet`
also passed.

Final M13 regression suite:
.venv\Scripts\python.exe -m pytest -q
540 passed, 3 deselected (live tests excluded by default marker)
```
...
- Backend calls are serialized by a call gate (ADR-027): the vendored client is not thread-safe (shared wasmtime PoW solver + per-turn parser seam), so concurrent OpenAI requests queue at the adapter boundary. Since M10 (ADR-037) the gate is PER-ACCOUNT — every account owns its own backend/PoW solver, so cross-account requests run in parallel and only requests routed to the SAME account queue; a long turn delays concurrent requests on that account until it finishes. Before the gate, the same traffic killed the process.
...
- Multi-account boundaries (M10, ADR-037): the account registry is in-memory — account health/cooldown/invalid state rebuilds fresh from config on every restart (a cooled-down or invalid account starts healthy again); accounts are config-driven only — since M12 (ADR-039) the running gateway supports enable/disable/reset per account (`POST /admin/accounts/{id}/...`), but runtime ADD/REMOVE and credential rotation remain deferred (would move credentials across the admin boundary); a conversation whose account died rebuilds through the ADR-020 path — since M11 (ADR-038) the FAILING request itself also fails over in-request when another usable account exists (see the M11 bullet below); `DSQG_COOKIES_FILE` cannot be combined with multi-account (per-account cookies arrive with persistence); "encrypted credentials" means in-memory `SecretStr` masking — at-rest encryption is deferred to the persistence milestone (SECURITY.md master-key design); a dead-link rebuild deliberately prefers its bound account while that account is cooldown-usable (a final 429 must not block the very conversation it failed), so the rebuilt turn may meet the rate limit again — the M9 retry budget and client backoff absorb it.
- Session failover boundaries (M11, ADR-038): failover covers ONLY final consequence-bearing PRE-byte failures (401- and 429-class — the account-scoped categories); mid-stream failures still become in-stream error envelopes (replaying after a committed byte would corrupt the wire), and fleet-wide categories (network/5xx/protocol/Cloudflare) never fail over (all accounts share one upstream — it would only double the load on the same outage). Failover is EXACTLY ONE bounded attempt per failing request — never chained. It is best-effort transparency, never an error transformer: no usable target, a disabled fleet, or the target's own session-creation failure surfaces the ORIGINAL error byte-identical (single-account deployments are byte-for-byte unchanged); a failed RE-RUN surfaces its own error (the latest truth) with the failover account's consequence recorded. A conversation committed on the failover account stays there — no migration back when the original account recovers (sticky from then on). The rehydrated turn pays one full-history prompt compilation (same cost as any ADR-020 rebuild). `session_failovers` is an in-memory per-process counter like all M9 metrics.
- Admin surface boundaries (M12, ADR-039): ALL `/admin/*` endpoints (page, reads, AND the lifecycle mutations) remain UNAUTHENTICATED, consistent with the M9/M10 precedent — the security boundary is the default local bind 127.0.0.1; anyone with access to the bound interface can disable/enable/reset accounts or read fleet metadata. Masking is STRUCTURAL (no view touches a `SecretStr`; credentials never enter the admin data path), so the exposure is operational state, never secrets. `/admin/sessions` is binding metadata only — message content and tool arguments are deliberately not surfaced. Settings are a read-only env echo (no runtime mutation — restart applies). Account lifecycle changes made through the UI do NOT survive a restart (the registry rebuilds from config, same as all M10 state). The page has no write capability beyond the three lifecycle POSTs.
- Docker intentionally not started (M13).

## Next action

**M12 COMPLETE (offline) — M0–M12 complete.** Admin UI shipped behind ADR-039: one self-contained HTML page at `GET /admin` (inline CSS + vanilla JS, zero external assets, five tabs — Dashboard / Accounts / Sessions / Metrics / Settings — 5-second auto-refresh) that is a pure stateless client of additive JSON endpoints: `GET /admin/summary` (health — the SAME closure `/health` uses — fleet counts by state, conversations, active sessions, uptime, six key metrics), `GET /admin/sessions` (binding metadata only — never message content or tool arguments), `GET /admin/settings` (effective config echo, secrets as presence only), plus account lifecycle mutations `POST /admin/accounts/{id}/disable|enable|reset` on the M10 router seams (disable releases conversation links; enable restores the flag only — invalid survives until reset; reset clears cooldown + failures + invalid; unknown id → 404 `ACCOUNT_NOT_FOUND`). Credentials stay STRUCTURALLY masked — no admin surface ever touches a `SecretStr`; sweep-tested with distinctive markers across all six admin read endpoints. Core API untouched: every M12 route is additive, `/v1/*` + `/health` + `/admin/metrics` + `/admin/accounts` byte-identical, and the suite grew 517 → 535 with ZERO re-pins. Auth stance unchanged: all `/admin/*` unauthenticated (local bind is the boundary — recorded residual risk). Runtime account add/remove and settings mutation stay deferred. No live acceptance run needed — every admin behavior is deterministic offline (fake backends) — but any running gateway instance predating M12 must be RESTARTED to serve it. **Next milestone is M13 (Docker/operator docs) — per the milestone gate, do NOT start M13 without the user's explicit go-ahead.**
