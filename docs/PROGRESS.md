...
- UI, Docker intentionally not started.

## Next action

**M11 COMPLETE (offline) — M0–M11 complete.** Session failover shipped behind ADR-038: a FINAL consequence-bearing pre-byte failure (401/429-class) on one account now records the M10 consequence, establishes EXACTLY ONE failover session on another usable account, rehydrates the request's FULL canonical history (tool history/IDs preserved by construction — same compilation path as the ADR-020 rebuild), and re-runs the same turn there. When another usable account exists, the failing request succeeds transparently (200); otherwise the ORIGINAL error surfaces byte-identical — failover is best-effort transparency, never an error transformer, and never chains. Mid-stream failures never fail over (committed-stream rule). New `session_failovers` metrics marker + one secret-free log line; no new config variables; `/health` and `/admin/accounts` unchanged; single-account deployments byte-for-byte unchanged. Full offline suite 517 passed, 3 deselected — 504 → 517 with exactly TWO planned re-pins of M10 flow tests that flip by design (the failing request now succeeds in-request; the 429 variant re-pinned with a disabled second account so the fleet-exhaustion and bound-rebuild pins survive). Live failover needs no dedicated acceptance run — it is simulated deterministically offline — but any running gateway instance predating M11 must be RESTARTED to serve it. **Next milestone is M12 (Admin UI) — per the milestone gate, do NOT start M12 without the user's explicit go-ahead.**
