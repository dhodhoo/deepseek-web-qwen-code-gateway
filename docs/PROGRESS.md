...
**Current milestone:** M10 — Multi-account router — COMPLETE (offline; routing/failover behavior is deterministic and pinned by tests). M0–M10 are complete; M11 (session failover) awaits the user's explicit go-ahead (milestone gate).

**State:** M10 delivered the ROADMAP multi-account items behind ADR-037: an in-memory account registry (`app/accounts.py` — `AccountRecord` rows with healthy/cooldown/invalid states, `disabled` derived; credentials live ONLY inside each account's own backend instance, so every admin surface is masked by construction); a thread-safe `AccountRouter` (least-active selection derived from the conversation store, unused accounts first, injectable clock, LAZY cooldown expiry); sticky conversation accounts (a live session keeps its account — even through a cooldown window; only invalid/disabled releases the link); final-failure consequences attached ONLY post-M9-budget (401-class → account INVALID + all its conversation links released to rebuild elsewhere; 429-class → bounded cooldown blocking NEW conversations only; other categories counter-only); `DSQG_ACCOUNT_TOKENS` multi-account config (mutually exclusive with `DEEPSEEK_AUTH_TOKEN`, fail-closed validation, `DSQG_ACCOUNT_COOLDOWN_SECONDS` default 300); per-account backend isolation (each account owns its vendored client/PoW/call gate); masked read-only `GET /admin/accounts`; fleet-aware `/health`. New suite `tests/test_m10_accounts.py` (46 tests). Suite 458 → 504 passed with ZERO re-pins — the 458-test regression pin held intact after one recorded ADR-037 refinement (dead-link rebuilds stay on their bound account while it is cooldown-usable).

**Previous milestone:** M9 — Reliability hardening — COMPLETE (offline; live behavior follows deterministically from the offline pins).
...
- Multi-account boundaries (M10, ADR-037): the account registry is in-memory — account health/cooldown/invalid state rebuilds fresh from config on every restart (a cooled-down or invalid account starts healthy again); accounts are config-driven only — runtime add/remove/credential rotation is M12; a conversation whose account died rebuilds on its NEXT request through the ADR-020 path (no named failover event/marker — M11 owns that); `DSQG_COOKIES_FILE` cannot be combined with multi-account (per-account cookies arrive with persistence); "encrypted credentials" means in-memory `SecretStr` masking — at-rest encryption is deferred to the persistence milestone (SECURITY.md master-key design); a dead-link rebuild deliberately prefers its bound account while that account is cooldown-usable (a final 429 must not block the very conversation it failed), so the rebuilt turn may meet the rate limit again — the M9 retry budget and client backoff absorb it.
- UI, Docker intentionally not started.

## Next action

**M10 COMPLETE (offline) — M0–M10 complete.** The multi-account router shipped behind ADR-037: in-memory account registry (credentials live only inside each account's own backend instance — admin surfaces masked by construction), least-active/healthy routing for NEW conversations (unused accounts first, store-derived active counts, no drifted counters), sticky conversation accounts (existing sessions never round-robin — a live session keeps its account even through a cooldown window), final-failure consequences (401-class → account invalid + its conversation links released to rebuild elsewhere; 429-class → bounded `DSQG_ACCOUNT_COOLDOWN_SECONDS` window blocking new conversations only), `DSQG_ACCOUNT_TOKENS` multi-account config with fail-closed validation (single-account deployments byte-for-byte unchanged), per-account backend isolation (own vendored client/PoW/call gate per account), masked read-only `GET /admin/accounts`, and fleet-aware `/health`. Full offline suite 504 passed, 3 deselected — the 458-test regression pin held with ZERO re-pins (one ADR-037 point-2 refinement recorded: dead-link rebuilds stay on their bound account while it remains cooldown-usable). Live multi-account behavior needs no dedicated acceptance run — routing/failover is simulated deterministically offline — but the running gateway instance predates M10 and must be RESTARTED to serve it (and to pick up any `DSQG_ACCOUNT_TOKENS` config). **Next milestone is M11 (session failover) — per the milestone gate, do NOT start M11 without the user's explicit go-ahead.**
