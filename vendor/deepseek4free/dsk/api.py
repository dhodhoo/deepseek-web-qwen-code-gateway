...
# [DSQG-VENDOR-PATCH] pkg_resources was removed from setuptools>=81 and never
# existed on stock Python 3.12+ venvs. Replaced with importlib.metadata; the
# check below only prints a warning, so behavior is unchanged.
from importlib.metadata import version as _dist_version, PackageNotFoundError as _DistributionNotFound
import sys
...
class DeepSeekAPI:
    ...
    def __init__(self, auth_token: str):
        ...
        try:
            # [DSQG-VENDOR-PATCH] pkg_resources -> importlib.metadata (see import note above)
            curl_cffi_version = _dist_version('curl-cffi')
